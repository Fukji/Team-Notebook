...
void hld(int cur, int cost, int prev){
	...
	int nextcost, nextchild = -1;
	int sz = adjlist[cur].size();
	for(int i = 0; i < sz; i++){
	...
	if(nextchild != -1){
		hld(nextchild, nextcost, cur);
	}

	for(int i = 0; i < sz; i++){
		if(adjlist[cur][i].first == prev || adjlist[cur][i].first == nextchild)
			continue;
		chainno++;
		hld(adjlist[cur][i].first, adjlist[cur][i].second, cur);
	}
}
